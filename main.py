...
def main():
    ...
    train_image_folder = '/Users/rupinajay/Developer/Amazon ML Challenge 24/train_iamges'
    test_image_folder = '/Users/rupinajay/Developer/Amazon ML Challenge 24/test_images'
    ...
    # Step 4: Filter train_data to only include valid images
    train_data_filtered = train_data[train_data['image_link'].apply(lambda x: Path(x).name).isin(valid_train_images)]
    print(f"Filtered training data: {len(train_data_filtered)} rows.")

    # Ensure the number of rows in train_data matches the number of extracted features
